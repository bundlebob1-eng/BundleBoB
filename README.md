...
- `site/studio.mjs` — service-led homepage, services, approach, and closing panel.
...
The page routes are `/`, `/services`, `/approach`, `/how-it-works`, `/solutions`, `/why-bundlebob`, `/resources`, three resource details, `/about`, `/contact`, and `/system`. Previous marketing routes redirect to their equivalents. `/system` is marked noindex. The generated sitemap includes the other twelve pages. `scroll3d/` is a retired experiment and is excluded from deployment.

Typography uses locally hosted Montserrat with native Georgia italic, and monospace for record identifiers. Font licenses are included in `assets/fonts/`. The redesigned pages use authored geometry and interface demonstrations rather than generated business photography. Legacy image assets remain in the asset directory. There are no analytics scripts or runtime packages. The original diagrams and reconciliation video remain authored code assets. See `docs/visual-direction.md` for the O.C. Tanner reference mapping and the approved font substitutions.
