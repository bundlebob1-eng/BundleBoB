...
- `site/editorial.mjs` — reference-led homepage, navigation, closing panel, and footer.
...
Typography uses locally hosted Montserrat and Roboto Slab, with native monospace for record identifiers. Font licenses are included in `assets/fonts/`. Three AI-generated business scenes are served locally as responsive WebP assets; their prompts are recorded in `docs/image-prompts.md`. There are no analytics scripts or runtime packages. The original diagrams and reconciliation video remain authored code assets. See `docs/visual-direction.md` for the O.C. Tanner reference mapping and the approved font substitutions.
