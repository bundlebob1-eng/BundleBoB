...
export async function build(){
 ...
 const assets=['site.css','site.js','theme.js','favicon.svg','og-image.png'];
 for(const asset of assets)await fs.copyFile(path.join(root,'assets',asset),path.join(output,'assets',asset));
 await fs.cp(path.join(root,'assets/video'),path.join(output,'assets/video'),{recursive:true});
